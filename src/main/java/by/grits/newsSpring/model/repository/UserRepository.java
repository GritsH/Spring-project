...
public interface UserRepository extends CrudRepository<User, String> {
    @Query("select email_address,user_password, role_type, added_at from users where email_address=?")
    User findByEmail(@Param("email_address") String email);

    @Query("select email_address, user_password, role_type, added_at from users where email_address=? and user_password=?")
    User findByEmailAndPassword(@Param("email_address") String emailAddress, @Param("user_password") String userPassword);

    @Query("insert into users(email_address, user_password, role_type, added_at) values(?,?,?,?)")
